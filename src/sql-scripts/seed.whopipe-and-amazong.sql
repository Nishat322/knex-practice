/*
We put BEGIN and COMMIT at the beginning of the file and at the end of the file so that 
PostgreSQL tranaction occurs it adds a BEGIN before each command and a COMMIT after each. 
Used to specify the start and end of a transaction. We set the BEGIN and COMMIT around 
multiple commands to combine them into one transaction. If one command works but the second 
one fails, the first one will also be undone.
*/
BEGIN;
